...
未達: 2 周目の専用挿絵（IMP-018。2026-09-12 に済）、BGM ジングル・最終ボス曲・シーケンサ拡張（次スプリント）。
...
| BUG-018 涙の川の渡れない沼（ユーザー報告「川の手前で飛び越えられずボットが停止」） | ステージ側と特定（`tools/check_gaps.mjs`: 縁から本物の物理で総当たり）。涙の川 段 A・第二章 W 区画の沼を 5 タイルに、踏み石 W を移動。全横スクロール面で渡れない縁 0 | check_gaps 出力「渡れない縁はありません」、golden 更新（stages.json 1 行）、デモ再収録（river 1200 → 2376） |
| BUG-019 人魚の見切れ（ユーザー報告） | クリップを水の絵の位置（waterY+6）に合わせた。攻撃範囲は不変。縦長スプライト案は IMP-026 に記録 | `cmp_stage4_26.png` |
| IMP-026 人魚スプライトの縦長化（予算 +30 の後、2026-09-12） | `enemy-mermaid` v2 を 1 リクエスト（233/280）。mermaid1 42×53 → 62×66、当たり判定 8×15 と頭の位置は固定、水面下は α0.3 | `mermaid_v2_wait.png`・`mermaid_v2_lunge.png`、golden gallery f0 更新 |
| IMP-018 2 周目専用挿絵 scene7（2026-09-12） | `ending-7` を 1 リクエスト（234/280）。256×256・32 色、§2.6 の帯内（flat 0.21 / dither 0.13 / 輪郭 302・0.23） | `ending_scene7_loop2.png` |
| IMP-028 鏡像の復活・BUG-022 鏡像の重なり（2026-09-13） | 22 秒で同じ軸へ復活、各階の鏡像は自分の階でだけ鏡になる。塔のデモを再収録（0 死 1728）。golden は塔と画廊を更新 | `test/mirror.test.js` 2 件、`assets/demo/stage-tower.json` |
| IMP-015 続き 3: ドレス優先（2026-09-13） | 落ちている dress / golddress へ寄り道、価値のある宝箱は足場へ登って開ける。第一章・第二章で各 1 着、進み −70 / −6、全 0 死。再生一致 8/8 | `assets/demo/stage1.json`・`stage2.json` |
| IMP-015 続き 2: 塔と第一章の死因（2026-09-12 夜） | 8 面の総死亡 3 → 0。鏡像は軸手前で止まって撃つ、蛆を吐く直前は跳ばない、跳ぶ先の敵も見る。ケーキの崖落ち（BUG-021）を修正。採用基準の死亡の重み 250 → 400。再生一致 8/8 | `assets/demo/*.json`、golden は塔のみ更新 |
| IMP-015 続き: デモボットの死因追跡（2026-09-12） | 8 面の総死亡 6 → 3、涙の川・工房・遊園地の進みが 2376 / 1965 / 2613 → 3897 / 2570 / 3780。針の群れの瞬間移動（BUG-020）を発見して修正。再生一致 8/8 | `assets/demo/*.json`、`test-results/shots/demo_*.png` |
| BUG-008 城の中景 B/C（2026-09-12） | 壁面の帯として 2 リクエスト（235〜236/280）。268×162 / 263×169、主中景 267×164 と 3 種連結。golden stage3 更新 | `cmp_stage3_20/50/80.png` |
...
| BUG-024（調査で発見 → ユーザー承認で生成） | 二段ジャンプのほうきは `w/h` 欠落の NaN 座標で一度も描かれていなかった。粗い 3 倍ドットは出さず、HD で生成（台帳 237〜238）: `props/broom`・`broom2` 100×38 / 100×37 セル・15 色・fits true（v1 171×36 は不採用、v2 159 幅の柄 59 列を `shorten_to` で削除）。主人公幅の 1.45 倍、2 コマ 12 Hz。**ユーザー評価（2026-09-13 15:51）: 現状のほうきは「間に合わせ。部品を合成しただけで、生きた動きとは言い難い」。いったんこのまま採用。実測: 2 コマは同じ raw の縦積み 2 本を別々に切り出したもので、最良の位置合わせ（dx 0, dy −1）でも不透明セル 1942 のうち 891（46%）が異なり、差は 1〜99 列の全幅に散る（穂先だけの制御された揺れではなく、モデルが 2 本を描き直した差）。加えて柄は同一列 59 列の削除で詰めた。** 改善は IMP-029 | `golden_stage1_f90_diff.png`（旧の未描画）、`broom_vs_idle_3x.png`（同一基準線・3 倍）、`broom_zoom6.png`、`broom_ingame_doublejump.png`。golden は 8 面の 90 フレーム目と工房・塔の 600 フレーム目を更新、軌跡・メニュー・画廊は不変 |
| カタログ | items 章の注記「炎は暫定で旧ドット絵」を実態（生成 2 コマ）に修正、chars 章のほうき行は素材があるときだけ。8 ページ error 0 | `catalog_chars_debt003.png` |
| BUG-006 私服の色分け（ユーザー指示 2026-09-13「bug-006 を直して」） | 派生を位置分類へ（ADR-0041）。白 (232,232,240)・紺 (38,57,122)・赤 (217,38,43) を追加し 15 コマ × 私服・金が 15 色、桃 (230,122,156) 残存 0、帽子領域の金 0。golden は涙の川・工房の 600 フレーム目（plain）と画廊（金）のみ更新 | `bug006_before_5x.png` → `bug006_after_5x.png`、`bug006_plain_all_3x.png`、`bug006_gold_all_3x.png`、`bug006_catalog_plain.png`・`_gold.png`。試行 6 回＋案 1 の 3 回の計測は ADR-0041（追記）。commit db580a6 →（案 1）本コミット |

検証: Vitest **152 件**（sprites.test 4 件 −、loader.test 4 件 ＋）、Playwright **14 件**（boot に 1 件追加。golden は画廊＋ほうきの出るフレーム）、`npm run build && node tools/check_dist.mjs` **285 読込**・bootMs 153・loaderWarnings 0・errors 0。生成 2 リクエスト（台帳 238/280、残 42）。デプロイはユーザー指示で実施（HANDOFF §1 の Version 参照）。
