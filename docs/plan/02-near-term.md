...
| BUG-024（調査で発見 → ユーザー承認で生成） | 二段ジャンプのほうきは `w/h` 欠落の NaN 座標で一度も描かれていなかった。粗い 3 倍ドットは出さず、HD で生成（台帳 237〜238）: `props/broom`・`broom2` 100×38 / 100×37 セル・15 色・fits true（v1 171×36 は不採用、v2 159 幅の柄 59 列を `shorten_to` で削除）。主人公幅の 1.45 倍、2 コマ 12 Hz | `golden_stage1_f90_diff.png`（旧の未描画）、`broom_vs_idle_3x.png`（同一基準線・3 倍）、`broom_zoom6.png`、`broom_ingame_doublejump.png`。golden は 8 面の 90 フレーム目と工房・塔の 600 フレーム目を更新、軌跡・メニュー・画廊は不変 |
