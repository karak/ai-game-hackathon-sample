...
| IMP-019 | 公開 URL の初回ロードが 6.6 秒（エッジ未キャッシュ、449 ファイル、転送 3.4 MB のうちフォント 2.07 MB）。フォントのサブセット化（使用文字のみ）、`dist/_headers` でハッシュ付き `/assets/*.js|.ttf` を `immutable` 長期キャッシュ、スプライトのアトラス化でリクエスト数を減らす。2 回目以降は 0.6 秒 | P2 |
