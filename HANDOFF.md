...
- リポジトリ: `/Users/yasushi/projects/poc-square`、**公開リポジトリ https://github.com/karak/ai-game-hackathon-sample**（`origin/main`、public）。HEAD は 2026-09-13 のセッション終了時点（Sprint R 観測性 → 人魚縦長 IMP-026・scene7 IMP-018・城の中景 BUG-008 → デモボットの死因追跡で 8 面 0 死・ドレス優先 → 鏡像の 22 秒復活 IMP-028・各階固定 BUG-022。`git log -12`）、`origin/main` へ push 済み、作業ツリー clean、公開 Version `a1deb407` と一致
- 検証: `npm test` Vitest **150 件**通過（architecture 5 件、構造化ログ 19 件、鏡像 2 件を含む）、`npm run e2e` Playwright **13 件**通過（構造化ログ `telemetry.spec` 2 件・8 面ボット自走・設定保存・デモ決定論・ポーズ／コンティニュー／2 周目／死亡ログ・実キー回帰・ロード時間・英語 UI・スマホ縦・強化魔法・偽装ゲームパッド・**golden**〔全 8 面の軌跡と画素ハッシュ＋画廊〕）。`npm run build` → `node tools/check_dist.mjs` で dist の素材 282 読込を確認
...
- スプリント履歴: E → F → G → H → I → J（公開）→ K（強化魔法）→ L（第二章再設計・カットイン）→ M（挿絵級の画風統一 IMP-022）→ N（毒沼と足場 IMP-021）→ O（テスター計測・パッド診断・Issue テンプレート）→ P（DDD コンテキスト整理とリファクタリング。DEBT-004 / DEBT-011）→ R（観測性: 構造化ログ・Worker `/api/log`・tail/集計。2026-09-12。同日〜13 日に予算 +30 の素材 3 件、デモボットの死因追跡〔8 面 0 死・ドレス優先〕、鏡像の復活と各階固定、ゲーム側の不具合 3 件 BUG-020/021/022 を修正）← Q（人手のいらない残課題: デモ全面収録 IMP-015・hurt2 BUG-007・`box` 定義 DEBT-007、敵回避ボット、沼 5 タイル化 BUG-018・人魚クリップ BUG-019）→ **次: Sprint R のユーザー受入 1 回（§4 先頭）。その後の候補は §4 の P2/P3**（`docs/plan/02-near-term.md` 末尾）
