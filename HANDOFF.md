...
7. BUG-006 私服の色分け（ユーザー指示 16:15、commit db580a6）: 派生を色 → 色から画素の位置分類へ（ADR-0041）。白・紺・赤の 3 色で襟・袖口・裾を縫い取り、15 コマ × 私服・金が 15 色、桃の残存 0。ユーザーの問い「色数の制約は表現力より重要か」→ 案 1（私服パレットの設計し直し: 髪の暗い 2 色を畳んで白の陰影を確保）で 15 色のまま解決、案 2（派生は 17 色まで）は不要。golden は涙の川・工房の 600 フレーム目と画廊だけ更新。証跡 `test-results/shots/bug006_*`。**デプロイは未実施**（公開版 d2ab28d0 は旧私服）
