...
function drawPlayerBody(e, g, cam, assets, dy) {
  ...
  if (e.broomT > 0 && assets.broom) { const br = Math.floor(e.broomT * 24) % 2 ? assets.broom2 : assets.broom; blit(g, br, e.facing < 0, e.centerX - br.w / 2 - cam.x, e.y + e.h - 4 - cam.y); } // 2 コマ 12 Hz。柄の中心線が足元の 2〜3 世界単位下に来る（props/broom 100×38 セル） // 旧文字列のほうきは w/h 欠落で NaN 座標になり描かれていなかった（BUG-024）。HD 素材ができるまで null
