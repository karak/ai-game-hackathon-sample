...
export function saveRuns(runs, storage) { try { storage?.setItem(RUN_KEY, JSON.stringify(runs)); return true; } catch { return false; } }
...
export function buildReport({ runs = [], deaths = [], settings = null, env = {}, summarizeDeaths = null, stages = 8 } = {}) {
  const rep = {
    report: REPORT_VERSION, at: new Date(env.now ?? Date.now()).toISOString(), version: env.version ?? 'dev',
