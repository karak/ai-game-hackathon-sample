...
| 細長い小物（ほうき・杖・柵）の長さ | 生成物の**全行が同一色の列**（柄・棒）を数えて中間の列を削り、目標幅にする（`specs.json` の `shorten_to`。再標本化なし） | 「幅 40%」「2 本横並び」で小さく描かせる（無視され、縦に積む）。主人公と同じ絵に入れて比率で測る（密度は変わらず、1 リクエスト無駄） |
...
| 「細長い物は絵の中の比率や『幅 40%』で長さを制御できる」 | モデルはキャンバス幅の約 8 割で描く。長さは後処理で同一列を削る（ほうき: 171→100 列、pitch 5 px） |
